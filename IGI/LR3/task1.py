"""
Task 1: Compute cosine using power series.
Lab: 1
Title: Cos Series Calculation
Version: 1.0
...
@timer_decorator
def compute_cos_series(x: float, eps: float) -> tuple[float, int]:
    """
    Compute the sum of the cosine series up to the specified precision.
...
def task1_main():
    ...
    sum_series, n = compute_cos_series(x, eps)
    math_fx = math.cos(x)
